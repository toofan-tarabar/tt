...
async function loadAllData() {
  ...
  try {
    ...
    state.iranian = iranianRaw
      .map(r => ({
        name: norm(r["نام راننده"]),
        plate: norm(r["شماره پلاک"]),
        destination: norm(r["مقصد"]),
        phone: norm(r["شماره تماس"]),
        entryDate: norm(r["تاریخ ورود"]),
        entryTime: norm(r["ساعت ورود"]),
        status: norm(r["وضعیت"])
      }))
    ...
    state.iraqi = iraqiRaw
      .map(r => ({
        name: norm(r["نام راننده"]),
        plate: norm(r["شماره پلاک"]),
        entryDate: norm(r["تاریخ ورود به ایران"]),
        status: norm(r["وضعیت"])
      }))
      .filter(r => r.name || r.plate);
...
function statusCounts() {
  const all = [...state.iranian, ...state.iraqi];
  return {
    total: all.length,
...
function renderDashboard() {
  ...
  // آخرین ورودها (۵ مورد آخر، ترکیب هر دو شیت به ترتیب معکوس)
  const combined = [...state.iranian, ...state.iraqi].slice(-5).reverse();
  ...
  if (combined.length === 0) {
    ...
    wrap.innerHTML = `
      <table class="recent-table">
        <thead><tr><th>راننده</th><th>پلاک</th><th>وضعیت</th></tr></thead>
        <tbody>
          ${combined.map(r => `
            <tr>
              <td>${r.name || "—"}</td>
...
function renderIranian(filterText = "") {
  ...
  const otherRows = filtered.filter(r => r.status !== STATUS_LOADING && r.status !== STATUS_WAITING);
  ...
  const renderGroup = (rows, numbered) => rows.map((r, i) => `
    <div class="record-card">
      <div class="record-top">
        <div class="record-name">
          ${numbered ? `<span class="record-num">${i + 1}</span>` : ""}
          ${r.name || "بدون نام"}
        </div>
        ${badgeFor(r.status)}
      </div>
  ...
  html += loadingRows.length ? `<div class="record-list">${renderGroup(loadingRows, true)}</div>` : emptyState("ti-loader-2", "موردی در این وضعیت نیست.");

  html += `<p class="section-label">منتظر بار <span class="section-count">${waitingRows.length}</span></p>`;
  html += waitingRows.length ? `<div class="record-list">${renderGroup(waitingRows, true)}</div>` : emptyState("ti-clock", "موردی در این وضعیت نیست.");

  if (otherRows.length) {
    html += `<p class="section-label">سایر وضعیت‌ها <span class="section-count">${otherRows.length}</span></p>`;
    html += `<div class="record-list">${renderGroup(otherRows, false)}</div>`;
  }
...
function renderAnnouncements() {
  ...
  const list = [...state.announcements].reverse();
  ...
  el.innerHTML = `<div class="record-list">${list.map(a => `
    <div class="announce-card">
      ...
      <div>
        ...
        ${a.date ? `<p class="announce-date">${a.date}</p>` : ""}
      </div>
    </div>
  `).join("")}</div>`;
}
...
// ------------------------------------------------------------
const pageTitles = {
  dashboard: ["داشبورد", "وضعیت کلی نوبت‌های بارگیری"],
