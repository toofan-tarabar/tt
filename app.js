...
function recentEntries(limit = 3) {
  const combined = [
    ...state.iranian.map(r => ({ ...r, _sort: toSortableDateTime(r.entryDate, r.entryTime) })),
    ...state.iraqi.map(r => ({ ...r, _sort: toSortableDateTime(r.entryDate, "") }))
  ];
  combined.sort((a, b) => b._sort.localeCompare(a._sort));
...
function renderDashboard() {
  ...
  const combined = recentEntries(3);
